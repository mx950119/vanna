		...
		computed: {
			...
			curSelectedDate: function() {
				...
				if(self.selectedDate) {
					...
					console.log(tmp[2] + tmp[0] + tmp[1])
					return tmp[2] + tmp[0] + tmp[1]
				} else {

					$.ajax({
						method: "POST",
						url: "http://192.168.0.192:8080/risk-control/api/varCorrelation/getNearDate",
						data: {
							token: self.token,
...
//		                 }
					//假数据END
						if(res.success && res.result) {
							self.selectedDate = res.result.substring(4,6) + '/' +  res.result.substring(6,8) + '/' + res.result.substring(0,4);
							return res.result
						}
...
//					        "netDatas": ["34", "34", "3"],
//					        "totalDatas": ["34", "34", "0"],
//					        "securityTypes": ["股票", "大宗商品期货", "股票期权期货"],
//					        "positionTypes": ["多仓", "空仓", "净仓", "总仓"]
//					    },
//					    "code": "200",
...
//					};
					...
					console.log(res.result)
					var tmpobj = {};
					_.each(res.result, function(v, k){
						...
						switch(k) {
							...
							case 'totalDatas':
								...
								_.each(v, function(v1,k1){
									tmparray.push(parseFloat(v1))
								})
								_.extend(tmpobj, {[k]:tmparray})
								break;
							default: 
								_.extend(tmpobj, {[k]:v})
								break;
